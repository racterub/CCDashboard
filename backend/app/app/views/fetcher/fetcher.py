...
def update_dashboard_by_summary(session, client, pairs):
    ...
    The dashboard table will start recording only from the day you start this service.
    ...
    change_sum = 0
    for i in pairs:
        coin_summary = session.query(Summary).filter(Summary.coin==i[0]).order_by(Summary.id.desc()).first()
        if (coin_summary):
